...
export default function NavBar() {
  ...
  return (
    <nav className="fixed top-4 inset-x-0 z-50 flex justify-center px-3 sm:px-4">
      <div className="w-full max-w-2xl flex items-center justify-between bg-white/70 backdrop-blur-xl rounded-full shadow-lg shadow-slate-200/50 border border-white/60 px-3 sm:px-5 py-2 sm:py-2.5">
        {/* Brand */}
          ...
          {links.map(({ href, label, icon }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`flex items-center gap-1.5 px-2.5 sm:px-4 py-1.5 rounded-full text-sm font-medium transition-all ${
                  active
